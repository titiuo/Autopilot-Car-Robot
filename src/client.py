...
class Client:
    ...
    def __init__(self, id):
        ...
        self.markers_positions = {i: Position() for i in range(10)}
        self.robots_positions = {i: Position() for i in range(5)}
        self.logic_state = "startup"
    ...
    def process_data(self, data):
        ...
        elif isinstance(data, Marker.Post):
            self.markers_positions[data.marker_id] = data.position
        elif isinstance(data, Robot.Post):
            self.robots_positions[data.robot_id] = data.position
        elif isinstance(data, Logic.PostState):
            self.logic_state = data.state        
...
if __name__ == "__main__":
    ...
    try:
        ...
        client.send(Logic.PostMilestone("reached_target"))
        ...
        time.sleep(1)
        logging.debug(f"logic state is {client.logic_state}")
